...
class Generator(nn.Module):
    def __init__(self, input_dim, hidden_dim, time_steps, lat_dim, long_dim):
        ...
        self.lat_long_mlp = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, time_steps * lat_dim * long_dim)
        )

    def forward(self, z):
        latent = self.main_mlp(z)
        lat_long_factors = self.lat_long_mlp(latent)
        lat_long_factors = lat_long_factors.view(-1, 1, self.time_steps, self.lat_dim, self.long_dim)  # Add channel dimension
        return lat_long_factors


class Discriminator(nn.Module):
    def __init__(self, input_channels, time_steps, lat_dim, long_dim, hidden_dim):
        super(Discriminator, self).__init__()
        self.conv_net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Conv3d(hidden_dim * 2, 1, kernel_size=(3, 3, 3), padding=1),
            nn.Flatten()
        )

        self.fc = nn.Sequential(
            nn.Linear(time_steps * lat_dim * long_dim, 1),
            nn.Sigmoid()
        )

    def forward(self, x):
        x = x.float()
        conv_out = self.conv_net(x)  # x should be (batch_size, channels, time_steps, lat_dim, long_dim)
        output = self.fc(conv_out)
        return output
