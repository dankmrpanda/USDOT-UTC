...
def bucket(lat, lon, time, lat_bucket_size, lon_bucket_size, time_bucket_period):
    df["lat_bucket"] = (lat // lat_bucket_size) * lat_bucket_size # buckets values specified
...
def preprocess(matrix, randomize):
...
def create_tensor():
    ...
    preprocess(main_matrix, True) # creates matrix, with true = randomize
